...
export default async function RegionPage({ params }) {
  ...
  return (
    <div className="min-h-screen bg-background-offwhite">
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-text-charcoal mb-6 font-poppins">
                {content.hero.heading || `Foster Agencies in ${regionName}`}
              </h1>
              <p className="text-xl text-gray-600 mb-8 font-inter">
                {content.hero.subheading || `Find accredited foster agencies in ${regionName}, ${countryName}`}
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
                <Button
                  ...
                  asChild
                >
                  <Link href="/contact">{content.hero.cta_primary?.text || "Get Foster Agency Support"}</Link>
                </Button>
                <Button
                  size="lg"
                  variant="outline"
                  ...
                  asChild
                >
                  <Link href="#cities">{content.hero.cta_secondary?.text || "Explore Cities"}</Link>
                </Button>
              </div>
            </div>
          </div>
      ...
      {(content?.benefits || content?.support || content?.training) && (
                ...
                {content?.benefits && (
                  <Card className="section-card rounded-modern-xl p-6">
                    ...
                    <ul className="space-y-3">
                      {Array.isArray(content.benefits.items) && content.benefits.items.map((item, index) => (
                ...
                {content?.training && (
                  <Card className="section-card rounded-modern-xl p-6 md:col-span-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {Array.isArray(content.training.programs) && content.training.programs.map((program, index) => (
            ...
            <div className="max-w-3xl mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <Button size="lg" variant="outline" className="glass text-white border-white hover:bg-white/10 px-8 py-6 text-lg font-semibold rounded-xl font-inter" asChild>
                  <Link href="#">
                    {content.cta.cta_secondary?.text || "Download Information Pack"}
                  </Link>
                </Button>
              </div>
            </div>
