...
function formatSlugToTitle(slug) {
  ...
  const safeSlug = typeof slug === 'string' ? slug : (slug || '').toString();
...
async function loadCountries() {
  try {
    ...
    if (!supabaseAdmin) {
      ...
      return [
        { id: '1', name: 'England', slug: 'england' },
        { id: '2', name: 'Scotland', slug: 'scotland' },
        { id: '3', name: 'Wales', slug: 'wales' },
        { id: '4', name: 'Northern Ireland', slug: 'northern-ireland' }
      ];
    }
...
async function loadRegionsForCountry(countrySlug) {
  try {
    ...
    if (!supabaseAdmin) {
      ...
      // Return mock data for development
      return [
        { id: '101', name: 'Greater London', slug: 'greater-london', country_id: '1' },
        { id: '102', name: 'West Midlands', slug: 'west-midlands', country_id: '1' },
        { id: '103', name: 'Greater Manchester', slug: 'greater-manchester', country_id: '1' }
      ];
    }
...
async function loadCitiesForRegion(countrySlug, regionSlug) {
  try {
    ...
    if (!supabaseAdmin) {
      ...
      // Return mock data for development
      return [
        { id: '1001', name: 'London', slug: 'london', region_id: '101' },
        { id: '1002', name: 'Birmingham', slug: 'birmingham', region_id: '102' },
        { id: '1003', name: 'Manchester', slug: 'manchester', region_id: '103' }
      ];
    }
...
async function loadAllLocations() {
  try {
    // Check if Supabase is configured
    if (!supabaseAdmin) {
      console.warn('Supabase not configured. Returning mock locations structure.');
    ...
    console.error('Error stack:', error.stack);
    // Return mock data structure as fallback
    return {
      'england': {
        id: '1',
        name: 'England',
